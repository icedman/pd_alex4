...
inline double fixtof(fixed x) { return (double)x / 65536.0; }

inline fixed fixmul(fixed x, fixed y) { return ftofix(fixtof(x) * fixtof(y)); }
...
inline int fixtoi(fixed x) { return fixfloor(x) + ((x & 0x8000) >> 15); }

inline fixed fixcos(fixed x) { return _cos_tbl[((x + 0x4000) >> 15) & 0x1FF]; }
