...
struct {
  ...
  int width, height;
    ...
    .count = 96, .rows = 10, .cols = 10, .width = 16, .height = 16};
...
BITMAP *create_bitmap(int w, int h) {
  ...
  b->h = h;
  b->w = w;
  b->tex = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGBA8888,
...
void draw_tile(BITMAP *bmp, int tile_id, int x, int y) {
  ...
  blit(bitmaps[I_TILESET], bmp, tile_x, tile_y, x, y, tileset_info.width,
       tileset_info.height);
}
...
void draw_sprite_h_flip(BITMAP *bmp, BITMAP *sprite, int x, int y) {
  ...
  SDL_SetRenderTarget(renderer, bmp->tex);
  SDL_RenderCopyEx(renderer, sprite->tex, NULL, &d, 0, NULL,
                   SDL_FLIP_HORIZONTAL);
}
...
void rotate_sprite_v_flip(BITMAP *bmp, BITMAP *sprite, int x, int y,
  ...
  SDL_SetRenderTarget(renderer, bmp->tex);
  SDL_RenderCopyEx(renderer, sprite->tex, NULL, &d, angle, NULL,
                   SDL_FLIP_VERTICAL);
}
...
void rectfill(BITMAP *bmp, int x1, int y1, int x2, int y2, int color) {
  ...
  SDL_SetRenderTarget(renderer, bmp->tex);
  SDL_SetRenderDrawColor(renderer, _palette[color].r, _palette[color].g,
                         _palette[color].b, 0xFF);
...
void draw_character_ex(BITMAP *bmp, BITMAP *sprite, int x, int y, int color) {
  ...
  SDL_SetTextureColorMod(sprite->tex, _palette[color].r, _palette[color].g,
                         _palette[color].b);
  SDL_SetRenderTarget(renderer, bmp->tex);
  SDL_RenderCopy(renderer, sprite->tex, NULL, &d);
}
...
void prepare_font(int id) {
  ...
  int texture_pitch = 0;
  SDL_LockTexture(texture, &converted_surface->clip_rect, &texture_pixels,
                  &texture_pitch);

  memcpy(texture_pixels, converted_surface->pixels,
         converted_surface->pitch * converted_surface->h);

  // Make background transparent
  uint32_t foreground_color =
      SDL_MapRGB(converted_surface->format, 0xFF, 0xFF, 0xFF);
  uint32_t background_color =
      SDL_MapRGB(converted_surface->format, 0x00, 0x00, 0x00);
  uint32_t transparent_color =
      SDL_MapRGBA(converted_surface->format, 0xFF, 0xFF, 0xFF, 0x00);
  ...
  for (int r = 0; r < 14; ++r) {
    for (int c = 0; c < 16; ++c) {
      font.charRects[char_code].x =
          1 + font.char_width * c + (padding_width * c);
      font.charRects[char_code].y =
          1 + font.char_height * r + (padding_height * r);
...
void textout_ex_helper(int pos, BITMAP *bmp, const char *s, int x, int y,
  ...
  SDL_SetTextureColorMod(font.texture, _palette[color].r, _palette[color].g,
                         _palette[color].b);
  SDL_SetRenderTarget(renderer, bmp->tex);
  ...
  for (size_t i = 0; i < length; ++i) {
    ...
    case '\n':
      ...
      break;
    default: {
      SDL_Rect r = {current_x, current_y, font.charRects[code].w,
                    font.charRects[code].h};
...
int text_length(const char *txt) { return strlen(txt) * font.char_width; }
int text_height() { return font.char_height; }
...
// file
int exists(const char *f) { return (access(f, F_OK) == 0); }
...
bool MouseBtn() { return (SDL_GetMouseState(NULL, NULL) & SDL_BUTTON_LMASK); }
...
void make_window(Toptions *o) {
  ...
  window = SDL_CreateWindow(GAME_TITLE_STR, SDL_WINDOWPOS_UNDEFINED,
                            SDL_WINDOWPOS_UNDEFINED, o->width, o->height,
                            window_flags);
...
void IncreaseZoom(Toptions *o) { SetZoom(zoom + 1, o); }
void DecreaseZoom(Toptions *o) { SetZoom(zoom - 1, o); }
...
void msg_box(const char *str) {
  pause_music(true);
  SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_INFORMATION, "Alex 4: Message", str,
                           NULL);
  pause_music(false);
}

void take_screenshot_platform(BITMAP *bmp, const char *filename) {
  SDL_Surface *tmp = SDL_CreateRGBSurface(0, bmp->w, bmp->h, 32, 0x00ff0000,
  ...
  size_t len;
  void *png = tdefl_write_image_to_png_file_in_memory(tmp->pixels, bmp->w,
                                                      bmp->h, 4, &len);
  SDL_SetRenderTarget(renderer, old);
  SDL_FreeSurface(tmp);
  if (!png)
