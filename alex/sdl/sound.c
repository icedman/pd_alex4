...
bool init_sound(Toptions *o) {
  ...
  if (Mix_OpenAudio(o->sound_freq, MIX_DEFAULT_FORMAT, MIX_DEFAULT_CHANNELS,
                    o->buffer_size) != 0) {
    return false;
  }
...
void set_sound_volume(int vol) { sound_vol = CLAMP(vol, 0, MAX_VOL); }

void set_music_volume(int vol) { music_vol = CLAMP(vol, 0, MAX_VOL); }

// plays a sample using user settings (from an index)
void play_sound_id_ex(int id, int vol, int freq, int loop) {
  if (!sounds_loaded || !sfx[id].chunk)
    return;
...
void play_sound_id(int id) { play_sound_id_ex(id, MAX_VOL, 1000, 0); }
