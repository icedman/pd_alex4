...
// set new edit mode
void set_edit_mode(int mode) { edit_mode = mode; }
...
char *get_edit_path_and_file() { return edit_path_and_file; }

// FIXME:
char *get_filename(const char *path) { return path; }
...
void draw_edit_mode(BITMAP *bmp, Tmap *map, int mx, int my) {
  ...
  if (edit_mode == EDIT_MODE_DRAW) { // draw map and HUD
    ...
    if (tx < map->width && ty < map->height) {
      if (edit_tile)
        draw_tile(bmp, edit_tile - 1, (tx << 4) - map->offset_x, ty << 4);
      rect(bmp, (tx << 4) - map->offset_x - 1, (ty << 4) - 1,
           (tx << 4) + 16 - map->offset_x, (ty << 4) + 16, gui_bg_color);
    }
    ...
    textprintf_ex(bmp, 1, 111, 4, -1, "EDITING: %s",
                  get_filename(edit_path_and_file));
    ...
    if (tx >= 0 && tx < 12 && ty >= 0 && ty < 8)
      rect(bmp, tx * 16 - 1 - ox, ty * 16 - 1 - oy, tx * 16 + 16 - ox,
           ty * 16 + 16 - oy, (logic_count & 4 ? gui_bg_color : gui_fg_color));
  } else if (edit_mode == EDIT_MODE_STATS) { // draw map properties
    int ty = 16;
    clear_to_color(bmp, 3);
    textprintf_ex(bmp, 1, 1, 1, -1, "%s (props)",
                  get_filename(edit_path_and_file));
    line(bmp, 0, 10, 159, 10, 1);
    textprintf_ex(bmp, 1, ty += 10, 1, -1, "%s", "Win by:");
    textprintf_ex(bmp, 1, ty += 10, 1, -1, "  1) reach exit (%s)",
                  (map->win_conditions & MAP_WIN_EXIT ? "X" : " "));
    textprintf_ex(bmp, 1, ty += 10, 1, -1, "  2) kill boss  (%s)",
                  (map->win_conditions & MAP_WIN_KILL_GUARDIAN ? "X" : " "));
    textprintf_ex(bmp, 1, ty += 10, 1, -1, "  3) kill all   (%s)",
                  (map->win_conditions & MAP_WIN_KILL_ALL ? "X" : " "));
    textprintf_ex(bmp, 1, ty += 10, 1, -1, "4) Boss level:  (%s)",
                  (map->boss_level ? "X" : " "));
...
void update_edit_mode(Tmap *map, BITMAP *bmp, int mx, int my, int mb) {
  ...
  if (edit_mode == EDIT_MODE_DRAW) {
    ...
    if (keypressed()) {
      ...
      case '1':
        if (ty < 7)
          map->dat[pos].item = (map->dat[pos].item == MAP_1UP ? 0 : MAP_1UP);
        break;
      case '2':
        if (ty < 7)
          map->dat[pos].item = (map->dat[pos].item == MAP_STAR ? 0 : MAP_STAR);
        break;
      case '3':
        if (ty < 7)
          map->dat[pos].item =
              (map->dat[pos].item == MAP_CHERRY ? 0 : MAP_CHERRY);
        break;
      case '4':
        if (ty < 7)
          map->dat[pos].item = (map->dat[pos].item == MAP_EGG ? 0 : MAP_EGG);
        break;
      case '5':
        if (ty < 7)
          map->dat[pos].item =
              (map->dat[pos].item == MAP_HEART ? 0 : MAP_HEART);
        break;
      case '6':
        if (ty < 7)
          map->dat[pos].item = (map->dat[pos].item == MAP_LOLA ? 0 : MAP_LOLA);
        break;
      // position tile types on map
      case 'q':
        if (ty < 7)
          map->dat[pos].type = (map->dat[pos].type == MAP_DEAD ? 0 : MAP_DEAD);
        break;
      case 'w':
        if (ty < 7)
          map->dat[pos].type = (map->dat[pos].type == MAP_EXIT ? 0 : MAP_EXIT);
        break;
      case 'e':
        if (ty < 7)
          map->dat[pos].type = (map->dat[pos].type == MAP_SPIN ? 0 : MAP_SPIN);
        break;
      case 'r':
        if (ty < 7)
          map->dat[pos].type = (map->dat[pos].type == MAP_BRK ? 0 : MAP_BRK);
        break;
      case 't':
        if (ty < 7)
          map->dat[pos].type =
              (map->dat[pos].type == MAP_WATER ? 0 : MAP_WATER);
        break;
      case 'y':
        if (ty < 7)
          map->dat[pos].type =
              (map->dat[pos].type == MAP_SURFACE ? 0 : MAP_SURFACE);
        break;
      // position enemies on map
      case 'Q':
        if (ty < 7)
          map->dat[pos].type =
              (map->dat[pos].type == MAP_ENEMY1 ? 0 : MAP_ENEMY1);
        break;
      case 'W':
        if (ty < 7)
          map->dat[pos].type =
              (map->dat[pos].type == MAP_ENEMY2 ? 0 : MAP_ENEMY2);
        break;
      case 'E':
        if (ty < 7)
          map->dat[pos].type =
              (map->dat[pos].type == MAP_ENEMY3 ? 0 : MAP_ENEMY3);
        break;
      case 'R':
        if (ty < 7)
          map->dat[pos].type =
              (map->dat[pos].type == MAP_ENEMY4 ? 0 : MAP_ENEMY4);
        break;
      case 'T':
        if (ty < 7)
          map->dat[pos].type =
              (map->dat[pos].type == MAP_ENEMY5 ? 0 : MAP_ENEMY5);
        break;
      case 'Y':
        if (ty < 7)
          map->dat[pos].type =
              (map->dat[pos].type == MAP_ENEMY6 ? 0 : MAP_ENEMY6);
        break;
      case 'U':
        if (ty < 7)
          map->dat[pos].type =
              (map->dat[pos].type == MAP_GUARD1 ? 0 : MAP_GUARD1);
        break;
      case 'I':
        if (ty < 7)
          map->dat[pos].type =
              (map->dat[pos].type == MAP_GUARD2 ? 0 : MAP_GUARD2);
        break;
    ...
    // move map
    if (key[KEY_LEFT])
      map->offset_x = MAX(0, map->offset_x - 4);
    if (key[KEY_RIGHT])
      map->offset_x = MIN((map->width << 4) - 160, map->offset_x + 4);

    // change map size
    if (key[KEY_INSERT] && key[KEY_RIGHT])
      change_map_size(map, 1, 0, SM_RIGHT);
    if (key[KEY_DEL] && key[KEY_RIGHT] && map->width > 10)
      change_map_size(map, -1, 0, SM_RIGHT);
    ...
    if (keypressed()) {
      ...
      if (kb == '2')
        map->win_conditions ^= MAP_WIN_KILL_GUARDIAN;
      if (kb == '3')
      ...
      if (kb == '4')
        map->boss_level ^= 1;
      if (kb == '5')
        get_string(bmp, map->name, 32, 8, 90, gui_fg_color, NULL);
    }
