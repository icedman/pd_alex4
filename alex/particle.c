...
void set_particle(Tparticle *p, int x, int y, double dx, double dy, int color,
                  int life, int bmp) {
  p->x = x;
  p->y = y;
  p->dx = dx;
...
void create_burst(Tparticle *ps, int x, int y, int spread, int num, int life,
  ...
  for (i = 0; i < num; i++) {
    ...
    if (p != NULL) {
      set_particle(p, x + rand() % spread - spread / 2,
                   y + rand() % spread - spread / 2,
                   ((double)(rand() % 100)) / 100.0 - 0.5,
                   ((double)(rand() % 100)) / 100.0 - 0.5, rand() % 4 + 1,
                   (life ? life : 70 - rand() % 50), bmp);
    }
  }
}
