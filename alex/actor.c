...
void draw_actor(BITMAP *bmp, Tactor *a, int x, int y) {
  ...
  if (a->status == AC_NORM) {
    ...
    if (a->direction)
      draw_sprite_h_flip(
          bmp, bitmaps[a->frames[a->frame] + (a->is_hit ? a->hit_offset : 0)],
          x, y - a->h - a->oy);
    else
      draw_sprite(
          bmp, bitmaps[a->frames[a->frame] + (a->is_hit ? a->hit_offset : 0)],
    ...
    if (a->direction)
      draw_sprite_vh_flip(bmp, bitmaps[a->frames[a->frame]], x,
                          y - a->h - a->oy);
    else
      draw_sprite_v_flip(bmp, bitmaps[a->frames[a->frame]], x,
                         y - a->h - a->oy);
...
void _crush_jump(Tactor *a, Tmap *m) {
  ...
  if (is_ground(m, a->x, a->y - 1))
    a->x++;
  if (is_ground(m, a->x + 31, a->y - 1))
    a->x--;
...
void update_actor_with_map(Tactor *a, Tmap *m) {
  ...
  if (a->status == AC_DEAD) {
    ...
    if (a->type == MAP_GUARD1) {
      ...
      if (p != NULL) {
        set_particle(p, a->x + 8 + rand() % 16, a->y - 8 - rand() % 16, 0, 0,
                     -1, 16, I_SMOKE1);
      }
    }
  } else if (a->status == AC_FLAT) {
    ...
    if (!is_ground(m, (int)a->x + 2, (int)a->y + 0)) {
      if (!is_ground(m, (int)a->x + 13, (int)a->y + 0)) {
        a->dy++;
        a->y += a->dy >> 2;
        while (is_ground(m, (int)a->x + 13, (int)a->y - 2)) {
          a->y--;
          a->dy = 0;
        }
        while (is_ground(m, (int)a->x + 2, (int)a->y - 2)) {
          a->y--;
          a->dy = 0;
        }
      }
    ...
    if (a->mode >= 125) {
      ...
      if (b != NULL) {
        ...
        set_bullet(b, a->x + (diff > 0 ? 8 : -12), a->y - 16,
                   (diff > 0 ? 3 : -3), 0, bitmaps[I_ENEMY6], 1);
        create_burst(particle, a->x + (diff > 0 ? 18 : -2), a->y - 8, 8, 8, 25,
                     I_PARTICLE_DUST);
        a->mode = 0;
    ...
    if (a->mode == 0) {
      ...
      while (is_ground(m, (int)a->x + 8, (int)a->y - 1)) {
        a->y--;
        hit = 1;
      }
      while (is_ground(m, (int)a->x + 24, (int)a->y - 1)) {
        a->y--;
        hit = 1;
      }
    ...
    */
    ...
    case 8: { // fire
      ...
      for (i = 0; i < 3; i++) {
        ...
        if (b != NULL) {
          set_bullet(b, a->x + 16, a->y - 32, rand() % 5 - 2, -(rand() % 5 + 2),
                     bitmaps[I_BULLET_1], 1);
    ...
    if (a->type == MAP_ENEMY2 || a->type == MAP_GUARD1) {
      if (!is_ground(m, (int)a->x + (a->direction ? a->w - 1 : 0),
                     (int)a->y + 2))
      ...
      if (!is_ground(m, (int)a->x + 2, (int)a->y + 0)) {
        if (!is_ground(m, (int)a->x + 13, (int)a->y + 0)) {
          a->dy++;
          a->y += a->dy >> 2;
          while (is_ground(m, (int)a->x + 13, (int)a->y - 1)) {
            a->y--;
            a->dy = 0;
          }
          while (is_ground(m, (int)a->x + 2, (int)a->y - 1)) {
            a->y--;
            a->dy = 0;
          }
        }
    ...
    if (is_ground(m, (int)a->x + (a->direction ? a->w : -1), (int)a->y - 2))
      a->direction = (a->direction ? 0 : 1);
  }
