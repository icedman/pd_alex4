...
void draw_player(BITMAP *bmp, Tplayer *p, int x, int y) {
  ...
  if (p->actor->status == AC_DEAD) {
    ...
    if (!p->actor->direction)
      ...
      rotate_sprite_v_flip(bmp, bitmaps[I_HERO_BALL], x, y - 16,
                           itofix(p->angle + 128));
  } else if (p->actor->status != AC_EAT) {
    if (p->actor->status == AC_FULL)
      head = bitmaps[I_HERO_FULL];
    else if (p->actor->status == AC_SPIT)
