...
char scroller_message[] =
    ...
    "Guide Alex to the exit of each level      Jump or shoot enemies picking "
    "up stars and cherries on the way      "
    "Use arrows to move Alex, ALT to jump and Left CTRL to shoot, or use a "
    "gamepad or joystick      "
    "Code and GFX by Johan Peitz      Music and SFX by Anders Svensson      "
    "SDL2 port by carstene1ns       "
    "Playdate port by icedman       ";
...
BITMAP *get_gameover_sign() { return bitmaps[I_GAME_OVER]; }

// returns pointer to the lets go bitmap
BITMAP *get_letsgo_sign() { return bitmaps[I_LETSGO]; }

#ifdef ENABLE_SHOOTER
// returns pointer to the space highscore table
Thisc *get_space_hisc() { return hisc_table_space; }
#endif

// sets the current map
void set_map(Tmap *m) { map = m; }

// returns the player actor
Tactor *get_alex() { return &actor[0]; }
...
void draw_status_bar(BITMAP *bmp, int y) {
  ...
  rectfill(bmp, 0, 240 - 20, 400, 240, 1);
...
void display_switch_in(void) { pause_music(false); }

// init the game
int init_game(const char *map_file) {
  int sz = 3;
  options.width = 400 * sz;
  options.height = 240 * sz;
  ...
  snprintf(filename, sizeof(filename), "config", homedir ? homedir : ".");
#else
  ...
  // init gfx
  log2file(" entering gfx mode set in config (%dx%d %s)", options.width,
           options.height, options.fullscreen ? "full" : "win");
  make_window(&options);
...
void show_lets_go() {
  ...
  draw_sprite(NULL, go, go->w / 2 - 35, 20);
  textout_outline_center(NULL, map->name, 100, 50);
  return;
...
void show_game_over() {
  ...
  draw_sprite(NULL, go, go->w / 2 - 50, 35);
  return;
...
void draw_custom_ending(BITMAP *bmp) {
  ...
  r = 70 + fixtoi(20 * fixcos(itofix(game_count >> 1)) +
                  20 * fixsin(itofix((int)(game_count / 2.7))));
  for (i = 0; i < 256; i += 32)
    draw_sprite(bmp, head,
                80 - head->w / 2 + fixtoi(r * fixcos(itofix(game_count + i))),
                60 - head->h / 2 + fixtoi(r * fixsin(itofix(game_count + i))));
...
void show_custom_ending() {
  ...
  while (!done) {
    ...
    while (cycle_count > 0) {
      ...
      // poll user
      poll_control(&ctrl);
      if (is_fire(&ctrl) || is_jump(&ctrl) || key[KEY_SPACE] ||
...
int *get_level_stats(int level) {
  ...
  level_stats[0] = _level;
  level_stats[1] = _lives;
  level_stats[2] = _stars;
  level_stats[3] = _cherries;
  return level_stats;
}
...
void show_cutscene(int level) {
  ...
  while (mode != 3) {
    ...
    while (cycle_count > 0) {
      ...
      if (((mode == 1) && (keypressed() || is_fire(&ctrl) || is_jump(&ctrl))) ||
          my_counter > 200) {
        mode = 2;
      }
...
void draw_select_starting_level(BITMAP *bmp, int level, int min, int max) {
  ...
  rectfill(bmp, 0, 0, 400, 240, 1);
  draw_sprite(bmp, bitmaps[I_ALEX_BG], 0, 0);
  draw_sprite(bmp, bitmaps[I_ALEX_BG], 150, 0);

  sprintf(buf, "%s %d %s", (level > min ? "<" : " "), level,
          (level < max + 1 ? ">" : " "));
  // clear_bitmap(stuff);
  render_push();
  render_scale(vec2(2,2));
  textout_centre_ex(bmp, buf, (200 / 2) / 2 + 1, 40 / 2, 1, -1);
  textout_centre_ex(bmp, buf, (200 / 2) / 2 - 1, 40 / 2, 1, -1);
  textout_centre_ex(bmp, buf, (200 / 2) / 2, 40 / 2 - 1, 1, -1);
  textout_centre_ex(bmp, buf, (200 / 2) / 2, 40 / 2 + 1, 1, -1);
  textout_centre_ex(bmp, buf, (200 / 2) / 2, 40 / 2, 4, -1);
  render_pop();
  // textout_centre_ex(bmp, buf, (200 / 2), 40 + 0, 4, -1);
  // stretch_sprite(bmp, stuff, 80 - 4 * stuff->w / 2, 30, 4 * stuff->w,
  //                4 * stuff->h);

  textout_centre_ex(bmp, "SELECT START LEVEL", 80 + 20, 90, 1, -1);
  textout_centre_ex(bmp, "SELECT START LEVEL", 79 + 20, 89, 4, -1);

#if ENABLE_SHOOTER
  if (options.one_hundred) {
    if (game_count & 32 || game_count & 16)
      draw_sprite(bmp, bitmaps[I_SHIP100], xpos, 2);
  } else
#endif
  {
...
int select_starting_level() {
  ...
  int done = 0;
  int counter = select_count;

  // draw_select_starting_level(swap_screen, start_level, 1,
  ...
  while (!done) {
    ...
    while (cycle_count > 0) {
      ...
      if (is_fire(&ctrl) && !counter) {
        done = 1;
        play_sound_id(S_MENU);
        start_level = -2;
      }
      if (is_jump(&ctrl) && !counter) {
        done = 1;
        play_sound_id(S_MENU);
        start_level = -1;
      }
      ...
      //     play_sound_id(S_MENU);
      //   }
      //   if (scancode == KEY_ESC || is_jump(&ctrl)) {
      //     done = 1;
      //     start_level = -1;
      //     play_sound_id(S_MENU);
      //   }
      // }
      if (!is_left(&ctrl) && !is_right(&ctrl))
        counter--;
      if (is_left(&ctrl) || is_right(&ctrl))
        counter = 500;
    ...
    for (y = 0; y < m->height; y++) {
      if (m->dat[x + y * m->width].type == MAP_ENEMY1) { // small human
        ...
        if (a != NULL) {
          ...
          a->ox = 2;
          a->oy = 2;
          a->flags = ACF_JUMPABLE | ACF_FLATABLE | ACF_HURTS | ACF_SHOOTABLE |
                     ACF_ROLLABLE;
          m->num_enemies++;
        }
      }
...
void new_level(const char *fname, int level_id, int draw) {
  ...
  if (player.actor->direction)
    tox = MAX(0, MIN(player.actor->x - 60, map->width * 16 - 160));
  else
    tox = MAX(0, MIN(player.actor->x - 85, map->width * 16 - 160));
  map->offset_x = tox;
  ...
  for (x = 0; x < map->width; x++) {
    for (y = 0; y < map->height; y++) {
      if (map->dat[x + y * map->width].item == MAP_STAR)
        player.stars++;
      if (map->dat[x + y * map->width].item == MAP_CHERRY)
        player.cherries++;
    }
  }
...
void update_player() {
  ...
  if (player.actor->status != AC_DEAD) {
    ...
    // handle firing
    if (is_fire(&ctrl) && player.actor->status == AC_FULL &&
        !player.eat_pressed) {
      ...
      if (player.ammo) {
        ...
        if (b != NULL) {
          set_bullet(b, (int)player.actor->x + 7, (int)player.actor->y - 14,
                     (player.actor->direction ? 4 : -4), 0, bitmaps[I_EGG2], 0);
          player.ammo--;
    ...
    if (is_ground(map, player.actor->x + x1, player.actor->y - 1)) {
      player.actor->x = old_x;
      if (player.actor->status == AC_BALL)
        play_sound_id(S_HIT);
      player.actor->status =
          (player.actor->status == AC_BALL ? AC_NORM : player.actor->status);
    };
    if (is_ground(map, player.actor->x + x2, player.actor->y - 1)) {
      player.actor->x = old_x;
      if (player.actor->status == AC_BALL)
        play_sound_id(S_HIT);
      player.actor->status =
    ...
    if (is_ground(map, player.actor->x + x1, player.actor->y - 15)) {
      player.actor->x = old_x;
      if (player.actor->status == AC_BALL)
        play_sound_id(S_HIT);
      player.actor->status =
    ...
    if (is_ground(map, player.actor->x + x2, player.actor->y - 15)) {
      player.actor->x = old_x;
      if (player.actor->status == AC_BALL)
        play_sound_id(S_HIT);
      player.actor->status =
    ...
    // check floor
    while (is_ground(map, player.actor->x + x1, player.actor->y - 1)) {
      player.actor->y--;
      player.jumping = 0;
    }
    while (is_ground(map, player.actor->x + x2, player.actor->y - 1)) {
      player.actor->y--;
      player.jumping = 0;
    }

    // check roof
    while (is_ground(map, player.actor->x + x1, player.actor->y - 15)) {
      player.actor->y++;
      player.actor->dy = 0;
    }
    while (is_ground(map, player.actor->x + x2, player.actor->y - 15)) {
      player.actor->y++;
      player.actor->dy = 0;
    }
    ...
    if (mp != NULL) {
      if (mp->type == MAP_SPIN && player.actor->status != AC_BALL) {
        Tmappos *m =
            get_mappos(map, tx + (player.actor->direction ? 1 : -1), ty - 1);
        if (m != NULL) {
          if (!m->mask) {
  ...
  for (b = 0; b < MAX_BULLETS; b++) {
    if (bullet[b].exist && bullet[b].bad == 0) {
      for (e = 1; e < MAX_ACTORS; e++) {
        if (actor[e].active && actor[e].status == AC_NORM &&
            (actor[e].flags & ACF_SHOOTABLE)) {
          if (check_bb_collision(bullet[b].x, bullet[b].y, 7, 6, actor[e].x,
                                 actor[e].y - actor[e].h, 16, actor[e].h)) {
            kill_actor(&actor[e]);
            map->num_enemies--;
            actor[e].direction = (bullet[b].dx > 0 ? 0 : 1);
            bullet[b].exist = 0;
            create_burst(particle, (int)bullet[b].x + 4, (int)bullet[b].y + 3,
                         4, 8, 0, -1);
  ...
  for (b = 0; b < MAX_BULLETS; b++) {
    if (bullet[b].exist && bullet[b].bad) {
      if (actor[0].status != AC_DEAD) {
        if (check_bb_collision(bullet[b].x + 1, bullet[b].y + 1,
                               bullet[b].bmp->w - 1, bullet[b].bmp->h - 1,
                               actor[0].x + 4, actor[0].y - 16, 8, 16)) {
          if (!player.wounded) {
            wound_player(&player);
            if (player.actor->status == AC_DEAD)
              play_sound_id(S_DIE);
          ...
          bullet[b].exist = 0;
          create_burst(particle, (int)bullet[b].x + 4, (int)bullet[b].y + 3, 4,
                       16, 0, -1);
        }
      }
    }
...
void check_alex_with_enemies() {
  ...
  if (actor[0].status != AC_DEAD) {
    for (e = 1; e < MAX_ACTORS; e++) {
      actor[e].is_hit = 0;
      if (actor[e].active && actor[0].status != AC_DEAD &&
          actor[e].flags & ACF_HURTS) {
        // first check if alex jumped on the enemy
        if (actor[0].dy >= 0 && actor[e].status == AC_NORM && player.jumping &&
            actor[0].status != AC_BALL && actor[e].flags & ACF_JUMPABLE) {
          if (check_bb_collision(actor[0].x + 2, actor[0].y + 12 - 16, 12, 8,
                                 actor[e].x, actor[e].y - actor[e].h,
                                 actor[e].w, 4)) {
            Tparticle *p;
            p = get_free_particle(particle, MAX_PARTICLES);
            if (p != NULL) {
              set_particle(p, actor[e].x + (actor[e].w >> 1), actor[0].y, 0, 0,
                           0, 5, I_PARTICLE_BOPP);
            }
            ...
            if (actor[e].flags & ACF_BOUNCEABLE) {
              ...
              player.dy = -5;
              // play_sound((actor[e].type == MAP_ENEMY5 ? sfx[S_KILL] :
              // sfx[S_STOMP]));
          ...
          if (actor[e].status == AC_NORM ||
              (actor[e].status == AC_FLAT && actor[0].status == AC_BALL)) {
            if (check_bb_collision(
                    actor[0].x + (actor[0].direction ? 0 : 8), actor[0].y - 16,
                    8, 16, actor[e].x + actor[e].ox, actor[e].y - actor[e].h,
                    actor[e].w, actor[e].h)) {
              int wounded = 0;
          ...
          // maybe he can eat them?
            ...
            if (check_bb_collision(actor[0].x + (actor[0].direction ? 16 : -8),
                                   actor[0].y, 8, 16, actor[e].x + 2,
                                   actor[e].y + 8, 12, 8)) {
              actor[e].active = 0;
              map->num_enemies--;
  ...
  if (camera_type == 0) { // centered camera, doesn't happen a lot
    ...
    if (pan) {
      if (p->actor->direction)
        tox = MAX(0, MIN(p->actor->x - 60, m->width * 16 - 208));
      else
        tox = MAX(0, MIN(p->actor->x - 85, m->width * 16 - 208));
    ...
    if (pan) {
      if (p->actor->direction)
        tox = MAX(0, MIN(p->actor->x - 34, m->width * 16 - 160));
      else
        tox = MAX(0, MIN(p->actor->x - 112, m->width * 16 - 160));

      d_tox = tox - m->offset_x;
      if (ABS(d_tox) > 70)
        step = 8;
      else if (ABS(d_tox) > 60)
        step = 7;
      else if (ABS(d_tox) > 50)
        step = 6;
      else if (ABS(d_tox) > 40)
        step = 5;
      else if (ABS(d_tox) > 30)
        step = 4;
      else if (ABS(d_tox) > 20)
        step = 3;
      else if (ABS(d_tox) > 1)
        step = 2;
      else if (ABS(d_tox) > 0)
...
int do_pause_menu(BITMAP *bg) {
  ...
  // show text
  textout_outline_center(
      bg, (editing ? "<<< EDITOR PAUSED >>>" : "<<< GAME PAUSED >>>"), 80, 40);
      ...
      if (!editing) {
        ...
        for (i = 1; i < MAX_ACTORS; i++) {
          if (actor[i].active) {
            ...
            else if (actor[i].x > map->offset_x - 32 - 16 &&
                     actor[i].x < map->offset_x + 160 + 32 + 16)
              update_actor_with_map(&actor[i], map);
          }
        }
        if (player.dy) {
        ...
        if (map->num_enemies == 0)
          map->win_conditions_fullfilled |=
              map->win_conditions & MAP_WIN_KILL_ALL;
...
void draw_title(BITMAP *bmp, int tick) {
  ...
  if (!playing_original_game)
    strcpy(start_string, "CUSTOM GAME");
  ...
  draw_sprite(bmp, bitmaps[I_ALEX_LOGO], 30 + 80 - w / 2, 30 - h / 2);
  ...
  draw_sprite(bmp, bitmaps[I_POINTER],
              x - 25 + fixtoi(3 * fixcos(itofix(tick << 2))),
              44 + menu_choice * step);
  // draw_sprite(bmp, bitmaps[I_POINTER], x - 25 + (int)(3 * cosf((tick <<
  // 2)*(float)M_PI/180)), 44 + menu_choice * step);
}

// gets a string from the user
// used for getting highscore names
int get_string(BITMAP *bmp, char *string, int max_size, int pos_x, int pos_y,
               int colour, Tcontrol *pad) {
  int i = 0, c;
  BITMAP *block =
      create_bitmap(text_length("w") * max_size + 2, text_height() + 2);
...
int do_main_menu() {
  ...
  while (status == GS_OK && loops++ < 1) {
    ...
    while (cycle_count > 0) {
      ...
      if (!is_any(&ctrl) && !key[KEY_UP] && !key[KEY_ESC] && !key[KEY_DOWN] &&
  ...
  if (status == GS_EDIT) {
    ...
    while (game_status == GS_OK || game_status == GS_LEVEL_DONE) {
      ...
      if (map->boss_level)
        start_music(MOD_BOSS_SONG);
      else
        start_music(MOD_LEVEL_SONG);
      ...
      if (status == GS_GAME_DIED) {
        ...
        if (level < MAX_LEVELS && playing_original_game) {
          ...
          for (i = 0; i < num_levels; i++) {
            if (options.cherries[i] != 100)
              ace = 0;
            if (options.stars[i] != 100)
              ace = 0;
          }
          if (ace)
            options.one_hundred = 1;
        }
        level++;
        show_cutscene(level);
        if (level == num_levels) {
    ...
    if (game_status == GS_GAMEOVER || game_status == GS_GAME_COMPLETE) {
      ...
      if (qualify_hisc_table(hisc_table, post)) {
        ...
        log2file(" player qualified for highscore (%d, %d)", post.level,
                 post.score);
...
int alex_main(int argc, char **argv) {
...
#ifdef __unix__
  // start logfile
  snprintf(full_path, sizeof(full_path), "%s/.alex4", homedir ? homedir : ".");
  ...
  // init game
  if (init_game(
          (playing_original_game ? "maps from datafile please" : argv[1]))) {
#ifndef NO_INTRO
    if (playing_original_game) {
